...
def get_clave_hacienda(doc, tipo_documento, consecutivo, sucursal_id, terminal_id, situacion='normal'):
    ...
    identification_code = doc.company_id.identification_id.code
    ...
    if token and (current_time - token_time < token_expire - 10):
        ...
        try:
            ...
            if 200 <= response.status_code <= 299:
                ...
                _logger.error('MAB - token_hacienda failed.  error: %s: %s' % (response.status_code, response.text))
...
def get_token_hacienda_company(company_id, tipo_ambiente):
    global last_tokens
    global last_tokens_time
    global last_tokens_expire
    global last_tokens_refresh
    ...
    if token and (current_time - token_time < token_expire - 10):
        ...
        try:
            ...
            if 200 <= response.status_code <= 299:
                ...
                raise Warning(_('Error Obteniendo el Token desde MH: %s: %s' % (response.status_code, response.text)))
        except requests.exceptions.RequestException as e:
            raise Warning(_('Error Obteniendo el Token desde MH. Excepcion %s' % (e)))
    return token_hacienda
...
def gen_xml_mr_43(clave, cedula_emisor, fecha_emision, id_mensaje,
    ...
    mr_monto_impuesto = round(monto_impuesto, 5)
    mr_detalle_mensaje = detalle_mensaje
    mr_total_factura = round(total_factura, 5)
    ...
    if codigo_actividad:
        sb.Append('<CodigoActividad>' + str(codigo_actividad).zfill(6) + '</CodigoActividad>')
...
def gen_xml_v43(inv, sale_conditions, total_servicio_gravado,
    ...
    if inv._name == 'pos.order':
        ...
        payment_methods_id = str(inv.payment_methods_id.sequence or '01')
    ...
    sb.Append('<CodigoActividad>' + (inv.economic_activity_id.code).zfill(6) + '</CodigoActividad>')
    ...
    sb.Append('<Ubicacion>')
    sb.Append('<Provincia>' + provincias.get(issuing_company.state_id.code, '1') + '</Provincia>')
        ...
        if receiver_company.name:
            ...
            if inv.tipo_documento == 'FEE':
                if receiver_company.vat:
                    sb.Append('<IdentificacionExtranjero>' + receiver_company.vat[:20] + '</IdentificacionExtranjero>')
            else:
                if inv.tipo_documento == 'NC' and id_code == '05':
                    if receiver_company.vat:
                        sb.Append(
                            '<IdentificacionExtranjero>' + receiver_company.vat[:20] + '</IdentificacionExtranjero>')
                if inv.tipo_documento == 'ND' and id_code == '05':
                    if receiver_company.vat:
                        sb.Append(
                            '<IdentificacionExtranjero>' + receiver_company.vat[:20] + '</IdentificacionExtranjero>')
                else:
                ...
                if tax_code.isdigit():
                    ...
                    if inv.tipo_documento != 'FEE':
                        if b.get('exoneracion'):
                            ...
                            # sb.Append('<PorcentajeExoneracion>' + str(int((b['tarifa'] / 100) * porcentaje_exoneracion)) + '</PorcentajeExoneracion>')
                            sb.Append('<MontoExoneracion>' + str(b['exoneracion']['montoImpuesto']) + '</MontoExoneracion>')
                            # sb.Append('<MontoExoneracion>' + str(round((b['monto'] / 100) * (porcentaje_exoneracion / 100) * 100, 5)) + '</MontoExoneracion>')
                            sb.Append('</Exoneracion>')
                    sb.Append('</Impuesto>')
    ...
    if inv.partner_id.vat == '4000042139':
        ...
        sb.Append('<TipoDoc>99</TipoDoc>')
        sb.Append("""<Numero>""" + str(numero_documento_referencia) + """</Numero>""")
        sb.Append("""<FechaEmision>""" + str(
            datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S-06:00')) + """</FechaEmision>""")
        sb.Append('<Codigo>99</Codigo>')
        sb.Append('<Razon>REFERENCIA FACTURA ICE</Razon>')
        sb.Append('</InformacionReferencia>')
    else:
...
def load_xml_data(invoice, load_lines, account_id, product_id=False, analytic_account_id=False):
    ...
    if activity_node:
        activity = invoice.env['economic.activity'].with_context(active_test=False).search(
            [('code', '=', activity_node[0].text)], limit=1)
    ...
    if partner:
        ...
        raise UserError(_('El proveedor asignado en el archivo XML no existe. Por favor agregarlo.'))
