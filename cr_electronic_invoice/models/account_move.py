    ...
    def _prepare_default_reversal(self, move):
        ...
        if self.reference_code_id and self.reference_document_id:
            result.update({
                ...
                'tipo_documento': 'NC' if move.type == 'out_invoice' else 'ND',
            })
        return result
...
class AccountInvoiceElectronic(models.Model):
    ...
    def get_default_tipo_documento_value(self):
        if self._context.get('default_type', ''):
            if self._context.get('default_type', '').startswith('out_'):
                return {
                    'out_invoice': 'FE',
                    'out_refund': 'NC'
                }[self._context['default_type']]
            else:
                return 'disabled'
        return False
    ...
    def get_tipos_documento(self):
        nc = [('NC', 'Nota de Crédito'),('ND','Nota de Débito')]
        if self._context.get('active_model', '') == 'account.move.reversal':
            if self._context.get('active_model', False) and self._context.get('active_id', False):
                type_last_doc = self.env[self._context['active_model']].browse(self._context['active_id']).move_id.type
        ...
        if self._context.get('default_type', '') and not self._context.get('from_print', False):
            type_doc = self._context.get('default_type', '').startswith('out_') and 'sale' or 'purchase'
            if type_doc == 'sale' and self._context['default_type'] == 'out_refund':
                return nc
            return type_docs[type_doc]
        return type_docs['sale'] + type_docs['purchase'] + nc
    name = fields.Char(string="Número", required=True, readonly=False)
    ...
    def sent_email_cron(self, limit=5):
        invoices = self.search([('invoice_date', '>', '2020-06-11'), ('state_tributacion', '=', 'aceptado'), ('date_invoice_sent', '=', False), ('type', 'in', ('out_invoice', 'out_refund',))], limit=limit)
        i = 0
        for inv in invoices:
            i += 1
            template = self.env.ref('account.email_template_edi_invoice', raise_if_not_found=False)
            ...
            if inv.partner_id and inv.partner_id.email:  # and not i.partner_id.opt_out:
                ...
                if attachment:
                    ...
                    if attachment_resp:
                        ...
                        try:
                            template.send_mail(inv.id, raise_exception=False, force_send=True)
    ...
    def _onchange_partner_id(self):
        ...
        if self._context.get('default_type', 'entry') == 'entry':
            return result
        self.payment_methods_id = self.partner_id.payment_methods_id
    ...
    def _check_hacienda_for_mrs(self, max_invoices=10):  # cron
        ...
        for inv in invoices:
            ...
            #     i.charge_xml_data()
            current_invoice += 1
            _logger.info('_check_hacienda_for_mrs - Invoice %s / %s  -  number:%s', current_invoice, total_invoices, inv.number_electronic)
    ...
    def _send_invoices_to_hacienda(self, max_invoices=10):  # cron
        ...
        invoices = invoice_env.search([('type', 'in', ('out_invoice', 'out_refund')),
                                                      ('state', 'in', ('posted', 'paid')),
                                                      ('number_electronic', '!=', False),
                                                      ('invoice_date', '>=', '2019-07-01'),
                                                      '|', ('state_tributacion', '=', False), ('state_tributacion', '=', 'ne')],
                                                      order='id asc', limit=max_invoices)
        invoices2 = invoice_env.search([('type', '=', 'in_invoice'),
                                                      ('tipo_documento', '=', 'FEC'),
                                                      ('state', 'in', ('posted', 'paid')),
                                                      ('number_electronic', '!=', False),
                                                      ('invoice_date', '>=', '2019-07-01'),
            ...
            if not inv.xml_comprobante or (inv.tipo_documento == 'FEC' and inv.state_tributacion == 'rechazado'):
                ...
                for inv_line in inv.invoice_line_ids:
                    ...
                    if False: #inv_line.product_id and inv_line.product_id.id == self.env.ref('cr_electronic_invoice.product_iva_devuelto').id:
                        ...
                        if not inv_line.product_uom_id or inv_line.product_uom_id.code in ('Al', 'Alc', 'Cm', 'I', 'Sp', 'Os', 'Spe', 'St', 'h'):  # inv_line.product_id.type == 'service'
    ...
    def post(self):
        for doc in self:
            if doc.company_id.skip_facturacion or doc.company_id.frm_ws_ambiente=='disabled':
                continue
            if doc.type not in ('out_invoice', 'out_refund', 'in_invoice', 'in_refund'):
                continue

            if doc.type == 'in_invoice' and doc.tipo_documento == 'FEC':
        ...
        return super(AccountInvoiceElectronic, self).post()
    ...
    def action_debit_note(self):
        ...
        nuevafacturaND = self.env['account.move'].create({
            ...
            'type': 'out_invoice',  # Out_invoice Factura de Cliente
            'state': 'draft',
            'tipo_documento':'ND',
    ...
    def get_invoice_sequence(self):
        ...
        elif self.type == 'in_refund' and self.xml_supplier_approval and self.tipo_documento == 'CCE':
            sequence = self.company_id.CCE_sequence_id.next_by_id()
        elif self.type == 'in_invoice' and self.xml_supplier_approval:
            sequence = self.company_id.CCE_sequence_id.next_by_id()
        elif self.type == 'in_invoice' and self.tipo_documento == 'FEC': #self.partner_id.country_id and \
