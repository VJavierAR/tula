				...
				if(vals['product_id']!=False):
					...
					if(producto):
						...
						if(p.x_studio_utilidad_precio_de_venta!=nueva and c==0):
							p.with_context(force_company=self.env.company.id).write({'x_studio_utilidad_precio_de_venta':nueva,'lst_price':precio,'nuevo_costo_facturacion_impuesto':precio_impuesto})
