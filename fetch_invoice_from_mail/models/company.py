                        ...
                        if filename.split('.')[-1:][0].lower() == "xml":
                            ...
                            if result and result != 'repetida':
                                try:
                                    ...
                                    num_lines = len(invoice_id.invoice_line_ids)
                                    ...
                                    if len(invoice_id.invoice_line_ids) > num_lines:
                                        n_lines = self.env['account.move.line']
                                        for lines_n in invoice_id.invoice_line_ids:
                                            if lines_n.desde_correo == True:
                                                n_lines += lines_n
                                        invoice_id.invoice_line_ids = n_lines
                                    invoice_id.tipo_documento = 'CCE'
